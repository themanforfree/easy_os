...
/// Use `ManuallyDrop` to ensure data is deallocated with an alignment of `BLOCK_SIZE`
...
impl BlockCache {
    ...
    fn raw_ptr(&self, offset: usize) -> *const u8 {
        addr_of!(self.cache.as_ref()[offset])
    }

    fn raw_ptr_mut(&mut self, offset: usize) -> *mut u8 {
        ...
        T: Sized,
    {
        ...
        let ptr = self.raw_ptr(offset) as *const T;
        unsafe { &*ptr }
    }
        ...
        T: Sized,
    {
        assert!(offset + core::mem::size_of::<T>() <= BLOCK_SIZE);
        self.modified = true;
        let ptr = self.raw_ptr_mut(offset) as *mut T;
        unsafe { &mut *ptr }
    }
...
pub fn get_block(block_id: usize, block_device: &Arc<dyn BlockDevice>) -> Arc<Mutex<BlockCache>> {
    BLOCK_CACHE_MANAGER
        .lock()
